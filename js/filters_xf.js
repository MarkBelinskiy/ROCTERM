...
function ajax_filters(action, cookie, class_xf, cache, data, widget, that) {
    ...
    $.post("/engine/ajax/filters_xf.php", {
        ...
        if (filters_ajax == "on") {
            ...
            if (url !== undefined) {
                if (!widget) $(fadename).fadeOut(250);
                $.ajax({
                    url: url,
...
function start_filters(sort) {
    ...
    $(".filters_d").each(function() {
        var key = $(this).attr("name"), val = $(this).val(), diapdef = eval(key + "_defmin")+"-"+eval(key + "_defmax");
        if (all_arr[key]) {
			// max
...
function clear_filters() {
	$(".filters_xf").each(function() {
		// diap
		if($(this).data('func') == "diap") {
			var key = $(this).attr("id");
			filters_sliders[key].noUiSlider.set([eval(key+"_defmin"), eval(key+"_defmax")]);
		}
		//check
...
$(document).ready(function() {
    ...
    $(document).on("click", ".filter_del", function(e) {
        ...
        // обнуляем по ID
        if ($('.filters_null[data-func="diap"][id="' + x + '"]').length) {
            $('.filters_null[data-func="diap"][id="' + x + '"]').click();
            return;
        }
    ...
    $(document).on("click", ".filters_null", function() {
        ...
        if (func == "diap") {
			filters_sliders[what].noUiSlider.set([eval(what+"_defmin"), eval(what+"_defmax")]);
			if (filters_what == "any")
                start_filters();
        }
    });
